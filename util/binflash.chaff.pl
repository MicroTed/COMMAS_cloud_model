...
#     $wmax[$curbin] = $wmax;
     ...
     $chaffchg[$curbin] = $chaffchg;
     $chaffchgtot = $chaffchgtot + $chaffchg;
     $chaffchg = 0;
...
# chaff net ion charge production (C):   3.11610296E+01
    if ( /^chaff net ion charge/ ) {
      chomp();
      @parts = split( / +/ );
      $chaffchg = $chaffchg + $parts[6];
      }
...
#    print ("curbin = $curbin; newbin = $newbin\n");
   ...
   for ($i = 1; $i < $curbin; $i++ ) {
    ...
    print OUT ( "Totals: $ictot, $cgptot, $cgntot, $triestot, ",
           "$icdistot ", $icdistot/Max($ictot,1), " $cgpchargetot $cgnchargetot $chaffchgtot \n" ); 
    print OUT ( "bin ICs CGPs CGNs Tries ICDIS  chg/ic    Chaff  CGPchg   CGNchg   Netchg ",
                "   NONIn    NONIp    INDn     INDp   wmax  Emax\n" );
   for ($i = 1; $i < $curbin; $i++ ) {
    printf OUT ( "%03d %3d %3d %3d %3d %8.3f %8.3f %8.3f % 8.3f % 8.3f % 8.3f % 8.3f % 8.3f % 8.3f %  8.3f %5.2f %e %e\n",
      $bintime[$i],  $ic[$i],  $cgp[$i],  $cgn[$i],  
      $tries[$i], $icdis[$i], $icdis[$i]/Max(1,$ic[$i]), $chaffchg[$i], $cgpcharge[$i],
