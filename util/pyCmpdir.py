...
RunIt = True #  debug switch, False:  will print out ncpdq commands, but not run the compression

ncpdq_string = "ncpdq -O -t 10 -7 -L 2"
...
#---------------------------------------------------------------------------------------------------
# Check_CMP:  examines the RMS errors for compressed version uncompressed fields.  

def check_cmp(orig_file, cmp_file):
    ...
    infile = netCDF4.Dataset(orig_file)
    cmpfile = netCDF4.Dataset(cmp_file)
    
    for var in check_fields:
        orig = infile.variables[var][...]
        cmp  = cmpfile.variables[var][...]    
        diff = orig - cmp    
        print "\nVar:  %s  Mean difference %f" % (var,diff.mean())
        print "Var:  %s  Mean absolute difference %f" % (var, N.abs(diff).mean())
        print "Var:  %s  Mean standard deviation %f" % (var, diff.std())
        
    infile.close()
    cmpfile.close()

#---------------------------------------------------------------------------------------------------
# Use a regular expression to match the given prefix with files having 3 numeral digits and then ".nc"
#
...
print """\n=====================================================================================================================
          ...
          pyCMPDIR:  A python script for compression of WRF/NCOMMAS netCDF ensembles using NetCDF Operators.

          This program will perform a lossy compression of netcdf files using the netCDF Operators (NCO) utility  
          'ncpdq' and uses python to walk down a directory structure to find files that match a prefix supplied 
          by the user. The code can either compress to 16 bit integers (which is practically lossless) or 8-bit
          integers which will generate ~5% errors for typical varibles.  Only netCDF variables are compressed 
