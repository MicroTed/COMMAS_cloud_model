...
# This version prints out stats for each flash (only for flashes, not every time step)
#
# Looks for the lines:
# $infile = 
...
#     print ("Time (s), Total Electrical Energy, Change in Energy, IC Flash, -CG Flash, +CG Flash, Positive Charge, Negative Charge, Net Charge, Max |E|, Net ion flux, Ion flux (top+bottom)\n");
#     print ("Time (s), Time (m), Total Electrical Energy, Change in Energy, IC Flash, -CG Flash, +CG Flash, Positive Charge, Negative Charge, Net Charge, Max |E|\n");
     print ("Time (s), Time (min), Flash Type, Total Electrical Energy, Flash Energy (J), Percent Change in Energy, Phistart (MV), Phiref (MV), Deltaphi (MV), Vert. Dipole Mom., CG Charge,Chg transfer, Dipole midpoint/charge altitude, Neg. Chan. Len., Pos. chan len., Tot. Chan. Len., Neg. Chan. Area., Pos. chan Area., Tot. Chan. Area. \n");
...
#       if ( $irst2 == 1 ) {  print ("HERE I AM: $newbin, $curbin\n"); }
    ...
    if ( /^NSTEP,/ ) {
     ...
     if ( $times > 0 ) {
#      print ("$times, $energy, $delenergy, $icyn, $cgnyn, $cgpyn, $chgpos, $chgneg, $chgnet, $efield, $ionnet, $iontb\n");
      }
...
#       if ( $irst2 == 1 ) {  print ("HERE I AM 3: $newbin, $curbin\n"); }
    ...
    if ( /^wmax/ ) {
     ...
     if ( /^phiref, phistart, delta-phi/ ) {
      @parts = split( / +/ );
      $phiref = $parts[4]/1.e6;
      $phistart = $parts[5]/1.e6;
      $deltaphi = $parts[6]/1.e6;
      }
     if ( /^dipole moment/ ) {
      @parts = split( / +/ );
      $dipole = $parts[5]*$parts[6]/abs($parts[5]);
#      print ("dipole: ",$parts[5]/$parts[6]," \n");
      $next = <INPUT>;
      chomp($next);
      @parts = split(/ +/,$next);
       $z = $parts[6];
     ...
     if ( /^Energy info: old,new,difference/ ) {
      ...
      $delenergy = $parts[5];
      $oldenergy = $parts[3];
      $percentchange = $parts[6];
       if ( $icyn > 0 || $cgnyn > 0 || $cgpyn > 0 ) { 
        if ( $icyn > 0 ) { $type = ' IC' }
        if ( $cgnyn > 0 ) { $type = '-CG' }
        if ( $cgpyn > 0 ) { $type = '+CG' }
       # $times = $times + 0.1; }
#      print ("$times, ",($times - $times%60)/60,":",$times%60,", $oldenergy, $delenergy, $percentchange, $icyn, $cgnyn, $cgpyn, $phistart, $phiref, $deltaphi\n");
#      print ("$times, ",($times - $times%60)/60,":",$times%60,", $oldenergy, $delenergy, $percentchange, $phistart, $phiref, $deltaphi\n");
#      printf ("%d, %02d:%02d, %5.1f, %5.1f, %5.2f\%, %5.2f, %5.2f, %5.2f \n", $times,($times - $times%60)/60,$times%60, $oldenergy/1.e6, $delenergy/1.e6, $percentchange, $phistart, $phiref, $deltaphi);
      if (( $icyn > 0 || $cgnyn > 0 || $cgpyn > 0 ) && $delenergy < 0 ) {
      printf ("%d, %7.2f, $type, %e, %e, %5.1f, %5.1f, %5.1f, %5.1f, %5.1f, (%5.1f), %e, %4.1f, %e, %e, %e, %e, %e, %e\n", 
         $times,($times)/60, $oldenergy, -$delenergy, $percentchange, $phistart, $phiref, $deltaphi, $dipole, $lgtchgpos, $lgtchgpos, $z, $chanlenneg, $chanlenpos, $chanlentot, $chanareaneg, $chanareapos, $chanareatot);
#      printf ("%d, %02d:%02d, $type, %e, %e, %5.1f, %5.1f, %5.1f, %5.1f, %5.1f, (%5.1f), %e, %4.1f, %e, %e, %e, %e, %e, %e\n", 
#         $times,($times - $times%60)/60,$times%60, $oldenergy, $delenergy, $percentchange, $phistart, $phiref, $deltaphi, $dipole, $lgtchgpos, $lgtchgpos, $z, $chanlenneg, $chanlenpos, $chanlentot, $chanareaneg, $chanareapos, $chanareatot);
       } else {
#      printf ("%d, %02d:%02d, $type, %e, %e, %5.1f, %5.1f, %5.1f, %5.1f, %5.1f, %5.1f, %e, %4.1f, %e, %e, %e, %e, %e, %e\n", 
#         $times,($times - $times%60)/60,$times%60, $oldenergy, $delenergy, $percentchange, $phistart, $phiref, $deltaphi, $dipole, $chgcenter, $cgabscharge, $z, $chanlenneg, $chanlenpos, $chanlentot, $chanareaneg, $chanareapos, $chanareatot);
       }
