...
# This version prints out energy and change at each time step, and flashes incurr an extra 0.1 seconds
# Used for figure in Mansell et al. 2010 to show ion drift response in/out of the domain to lightning flashes
#
# Looks for the lines:
# $infile = 
...
#    }
    ...
    $dt = 4;
     ...
     print ("Time (s), Time (min), Total Electrical Energy, Change in Energy, IC Flash, -CG Flash, +CG Flash, Positive Charge, Negative Charge, Net Charge, Max |E|, Net ion flux, Ion flux (top+bottom)\n");
...
#     if ( $times > 0 ) {
     if ( $energy > 0 ) {
      print ("$times, $timemin, $energy, $delenergy, $icyn, $cgnyn, $cgpyn, $chgpos, $chgneg, $chgnet, $efield, $ionnet, $iontb\n");
      }
     if ( $energylgt > 0 ) {
      print ("$timelgt, $timelgtmin, $energylgt, $delenergylgt, $icyn, $cgnyn, $cgpyn, $chgpos, $chgneg, $chgnet, $efield, $ionnet, $iontb\n");
      }
