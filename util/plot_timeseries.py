#!/Volumes/boot_clone/opt/local/anaconda/bin/python
# #!/opt/local/anaconda/bin/python
# #!/opt/local/anaconda.updated/bin/python
# #!/Users/Shared/anaconda2bak/bin/python
...
# for a set of plots where the run name varies only by a number in the middle, set the runhead to everything
# before the number, and tail to everything after. For example, jun29z100cnnosat -> runhead = 'jun29z', tail = cnnosat
# Then use vtlist for all the different values
runhead = 'jun29z2m11is'
tail = 'cnrescni19force2nosat2dtx5ir10max'

#runhead = 'may29tak1000cn'
#tail = 'vtr'

datatype = 1 # 1 = bin.ion; 2 = coldpool; 3 = precip; 4 = binproc
# set index
...
# for fall speed tests:
#vtlist = ['07','08','09','10','11','12','13']
#label_list = ['0.7 Vt','0.8 Vt','0.9 Vt','1.0 Vt','1.1 Vt','1.2 Vt','1.3 Vt']

# for CCN sensitivity:
vtlist = ['50','100','250','500','1000','1500']
label_list = ['50 CCN','100 CCN','250 CCN','500 CCN','1000 CCN','1500 CCN']
color_list = ['skyblue','dodgerblue','blue','black','red','lightcoral']
...
if ( datatype == 1 ):
   ...
   elif ( index == 52 ): # updraft volume w > 5
       pp = outputhead + '.bin.udv5.pdf'
...
for filename in files:
   ...
   if ( datatype == 1 ):
      dat = pd.read_csv(filename,header=4,sep=',')
   elif ( datatype == 4 ):
...
if ( datatype == 1 ):
   if ( index == 52 ):
       plt.ylabel('Updraft Volume ($>$ 5m/s) (km$^{3}$)')
   elif ( index == 53 ):
...
plt.axis([20,90,ymin, ymax])
if ( index == 66 or index == 67 ):
    plt.axis([20,90,250, 950])
plt.minorticks_on()
