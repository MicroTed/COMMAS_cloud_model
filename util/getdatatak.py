...
for iname in range(1,narg):
    ...
    if os.path.isfile(filename+'.000.nc'):
       print 'Single netcdf file'
       filetype = 1
    elif os.path.isfile(filename+'.000000.nc'):
       print 'Multiple netcdf files'
       filetype = 2
    else:
    ...
    if ( filetype == 1 ):
       ...
       ncfile = netcdf.Dataset(filename+'.000000.nc','r') 
    ...
    #      continue
       if ( filetype == 2 ):
          ncfile.close
          # print 'opening file ' + filelist[t]
          ncfile = netcdf.Dataset(filelist[t],'r') 
          times = ncfile.variables['TIME']
          currtime = times[0]
          #print 'currtime = ', currtime
          #print 'currtime.shape = ', currtime.shape[0]
          thv2d = ncfile.variables['THETA-V'][0,0,:,:]
          th2d = ncfile.variables['TH'][0,0,:,:]
          t2d = ncfile.variables['T'][0,0,:,:]
       else:
    ...
    #   print times[t]/60, avevaltem,numtem,sumtem,aveval1,num1,sum1
       fileout.write( str(currtime/60)+','+str(avevaltem)+','+str(numtem)+','+str(sumtem)+','+str(aveval1)+','+str(num1)+','+str(sum1)+'\n')
