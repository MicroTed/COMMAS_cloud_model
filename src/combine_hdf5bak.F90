...
! variables vars
  ...
  integer, dimension(:), allocatable :: mx,mxp1,my,myp1
...
!  real, dimension(:), allocatable :: var1D, time, dt
  ...
  usehdf5 = .true.
  one_file_at_a_time = .false. !  .true.  ! false for one var at a time
  one_time_at_a_time = .true.
  one_time = .false.
...
! get dimensions here
    ...
    status = NF90_INQ_DIMID(ncid(i),"dim2",DimIDdim2)
    status = NF90_INQUIRE_DIMENSION(ncid(i),DimIDdim2,len=dim2)
    status = NF90_INQ_DIMID(ncid(i),"dim3",DimIDdim3)
    status = NF90_INQUIRE_DIMENSION(ncid(i),DimIDdim3,len=dim3)
...
! get dimensions here
    ...
    status = NF90_INQ_DIMID(ncid(ii),"dim2",DimIDdim2)
    status = NF90_INQUIRE_DIMENSION(ncid(ii),DimIDdim2,len=dim2)
    status = NF90_INQ_DIMID(ncid(ii),"dim3",DimIDdim3)
    status = NF90_INQUIRE_DIMENSION(ncid(ii),DimIDdim3,len=dim3)
...
!        Var3D(vid) = 4
      ...
      ELSE
        IF (VarDim(3) == 1) THEN
          IF ( usehdf5 ) THEN
            CALL READ_WRITE_VAR2D_hdf5(vid)
          ELSE
...
!        Var3D(vid) = 4
        IF ( ktime == ntbeg ) CALL DEF_VAR3D(vid)
...
!-----------------------------------------------------------------------
   ...
   IF ( .not. one_file_at_a_time ) THEN
...
!         DO i = 1,ntiles_x
         DO i = nllc,nllc+ntiles_x-1
           ...
           status = NF90_GET_VAR(ncid(i),vid,var1D(istart:istop),(/k/)) 
         ENDDO
...
! #####################################################################################
   ...
   SUBROUTINE READ_WRITE_VAR2D_hdf5(vid)
...
! #####################################################################################
   ...
   SUBROUTINE READ_WRITE_VAR3D(vid)
