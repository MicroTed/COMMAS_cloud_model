...
!  real    :: dz
  ...
  integer i,j,k, it, nt, count, ix,jy, countic, countcgn, countcgp
  ...
  integer  :: imake8kmfed    = 0
  real     :: dxfed = 8000.
  integer  :: index_FLSH8KM,index_flshfedic, index_flshfedcgn, index_flshfedcgp
  integer  :: i8kmfedinterval = 60 ! reset interval for GLM FED
  integer  :: ioffset4km = 4 ! default assume dx=1km

  NAMELIST /com2v5d_param/               &
!                      visinterval,       &
                      recalcdbz,          &
                      iusewetgraupelv5d,  &
                      iusewethailv5d,   &
...
! READ IN SPECIAL FLAGS
  ...
  rewind(15)
  read(15,NML=com2v5d_param,iostat=istat)
  close(15)
      IF ( istat .ne. 0 ) THEN
        write(0,*) 'READ_NAMELIST: com2v5d_param namelist not found -- using default values'
      ENDIF
...
!  IF( .not. READ_NAMELIST(run_file(1:length),'ATTRIBUTE') ) print *, 'COMMAS_INIT:  PROBLEM READING NAMELIST'
    ...
    IF ( start .gt. Maxval(tarray(1:nt)) ) THEN
...
5  INQUIRE(file=v5dfilename, exist=file_exist)
   ...
   IF ( file_exist ) THEN
     ...
     v5dfilename = prefix(ibeg:iend)//trim(number)//'.'//dup//'.v5d'
     onedfilename = prefix(ibeg:iend)//trim(number)//'.'//dup//'.'//column//'.1d'

     GOTO 5
   ELSE
     write(6,*) 'COMMAS:  CREATE NEW VIS5D FILE: ', v5dfilename ! prefix(1:length)//trim(number)
     gx     = GET_VARIABLE_INDEX(gd, 'XC')
     gy     = GET_VARIABLE_INDEX(gd, 'YC')
     gz     = GET_VARIABLE_INDEX(gd, 'ZC')
     call Vis5DInit( gd, gd%var(gx), gd%var(gy), gd%var(gz) , v5dstridex, v5dstridez, mpisep, iflag=1)
...
!  CALL GET_VARIABLE(gd, 'NZ',       nz)
  ...
  IF ( imake8kmfed /= 0 )  THEN
    ioffset4km = Nint(dxfed/dx)
    write(0,*) 'ioffset4km = ',ioffset4km
    index_FLSH8KM = GET_VARIABLE_INDEX(gd, 'FLSH8KM')
    index_FLSHFEDIC =  GET_VARIABLE_INDEX(gd, 'FLSHFEDIC')
    index_FLSHFEDCGN = GET_VARIABLE_INDEX(gd, 'FLSHFEDCGN')
    index_FLSHFEDCGP = GET_VARIABLE_INDEX(gd, 'FLSHFEDCGP')
    allocate ( glm(1:nx,1:ny) )
    glm(:,:) = 0
  ENDIF
...
! fake 2D GLM 
    IF ( imake8kmfed /= 0 )  THEN
      ...
      IF( mod(time,i8kmfedinterval) == 0 ) THEN
        gd%var(index_FLSH8KM)%flt2d(:,:) = 0.0
        glm(:,:) = 0
        write(0,*) 'resetting FLSH8KM'
      ENDIF


      ! gd%var(index_FLSH8KM)%flt2d
        DO jy = ioffset4km/2, ny-1, ioffset4km
          DO ix = ioffset4km/2, nx-1, ioffset4km
!           write(0,*) 'FLSH8KM: ix, jy = ', ix,jy
              countic = 0
              countcgn = 0
              countcgp = 0
              DO j = jy-ioffset4km/2+1, Min(ny-1, jy+ioffset4km/2 )
               DO i = ix-ioffset4km/2+1, Min(nx-1, ix+ioffset4km/2 )
               
                IF ( index_FLSHFEDIC > 1 ) THEN
                  countic = Max(countic, Nint( gd%var(index_FLSHFEDIC)%flt2d(i,j) ) )
                ENDIF
                IF ( index_FLSHFEDCGN > 1 ) countcgn = Max(countcgn, Nint( gd%var(index_FLSHFEDCGN)%flt2d(i,j) ) )
                IF ( index_FLSHFEDCGP > 1 ) countcgp = Max(countcgp, Nint( gd%var(index_FLSHFEDCGP)%flt2d(i,j) ) )
               ENDDO
              ENDDO
          
              DO j = jy-ioffset4km/2+1, Min(ny-1, jy+ioffset4km/2)
               DO i = ix-ioffset4km/2+1, Min(nx-1, ix+ioffset4km/2)
                 glm(i,j) = glm(i,j) + countic + countcgn + countcgp ! adds 1 to flshr if flash goes through the column.
!                 gd%var(index_FLSH8KM)%flt2d(i,j) = gd%var(index_FLSH8KM)%flt2d(i,j) + countic + countcgn + countcgp ! adds 1 to flshr if flash goes through the column.
               ENDDO
              ENDDO
      ...
      DO jy = 1,ny
      DO ix = 1,nx
        gd%var(index_FLSH8KM)%flt2d(ix,jy) = glm(ix,jy)
      ENDDO
      ENDDO
