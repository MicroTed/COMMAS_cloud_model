/* $Id: v5d.h,v 1.1.1.1 2010/07/29 18:18:52 mansell Exp $ */
...
#define MAXROWS     300
#define MAXCOLUMNS  300
#else
#define MAXROWS     800
#define MAXCOLUMNS 4000
#endif
...
extern void v5dOpen( const char *name,
                      int numtimes, int *numvars,
                      int nr, int nc, int nl[],
                      ...
                      float proj_args[],
                      int vertical,
                      float vert_args[], int istuff[] );


extern int v5dWrite( int time, int var, const float data[] );

extern int v5dWriteAppend( int time, int var, const float data[], const int timestamp, const int dates );


extern int v5dClose( void );

extern int myv5dUpdate( void );
...
extern int myv5dUpdateFile( v5dstruct *v );
