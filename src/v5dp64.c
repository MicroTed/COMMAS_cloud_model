...
#define F77_FUNC_(name,INTL) name__

#else

#define F77_FUNC(name,INTL) name
#define F77_FUNC_(name,INTL) name
...
/*
 * Update a v5d file header which was opened with open_v5d_file() or
 * create_v5d_file().
 * Input: f - file descriptor
 * Return:  1 = ok, 0 = error
 */
int myv5dUpdateFile( v5dstruct *v )
{
   int status = 1;
   ...
   if (v->Mode=='w') {
    ...
    /*  close(v->FileDesc); */
      ...
      printf("Error in myv5dUpdateFile: bad v5dstruct argument\n");
      return 0;
   }
 /*  v->FileDesc = -1;
 *  v->Mode = 0; 
 ...
 * Return:  1 = ok, 0 = error
 */
int v5dWriteAppend( int time, int var, const float data[],const int timestamp,const int dates )
{
   if (Simple) {
      if ( time==Simple->NumTimes+1) { 
          Simple->NumTimes++;
          Simple->TimeStamp[time-1] = timestamp;
          Simple->DateStamp[time-1] = dates;
       }
      if (time<1 || time>Simple->NumTimes) {
         printf("Error in v5dWrite: bad timestep number: %d\n", time );
         return 0;
...
/*
 * Update a v5d file head the last grid of a time level has been written to it.
 * Return:  1 = ok, 0 = error
 */
int myv5dUpdate( void )
{
   if (Simple) {
     int ok = myv5dUpdateFile( Simple );
 /* //   v5dFreeStruct( Simple );
   */
     return ok;
   }
   else {
     printf("Error: myv5dUpdate: no file to update\n");
     return 0;
   }
}
 ...
 * Return:  1 = ok, 0 = error
 */
int F77_FUNC(v5dwriteappend,V5DWRITEAPPEND)
          ( const int *time, const int *var, const float *data, const int *timestamp, const int *dates )
{
   return v5dWriteAppend( *time, *var, data, *timestamp, *dates );
}



/*
 * Update header of a simple v5d file.
 */
int F77_FUNC(myv5dupdate,MYV5DUPDATE)
   (void)
{
   return myv5dUpdate();
}
...
/*
 * Open a v5d file.  See README file for argument descriptions.
 * Return:  1 = ok, 0 = error.
 */
int F77_FUNC(myv5dopen,myV5DOPEN)
           ( const char *name, int *numtimes, int *numvars,
             int *nr, int *nc, int nl[],
             char varname[][10],
             int timestamp[], int datestamp[],
             int *compressmode,
             int *projection,
             float proj_args[],
             int *vertical,
             ...
             int istuff[])
{
   char filename[100];
   char names[MAXVARS][10];
   int i, maxnl, args, time, var;
//   int mynumvar = *numvars;
...
/*    v5dOpen( filename, *numtimes, &mynumvar, *nr, *nc, nl,
                     (char(*)[10]) names, timestamp, datestamp,
                     *compressmode,
  ...
  */   
  ...
  //   return 0;
      ...
      printf("Opened %s for editing\n", name );
   }


  // v5dPrintStruct ( Simple );
 //     printf("NumVars = %d\n",  Simple->NumVars );
 //     printf("NumTimes = %d\n", Simple->NumTimes );


   *numtimes = Simple->NumTimes;
   *numvars  = Simple->NumVars;
   istuff[0] = Simple->NumVars;
   *nr      = Simple->Nr;
   *nc      = Simple->Nc;
//   maxnl = nl[0];
 ...
 //        maxnl = nl[var];
 //     }
      nl[var] = Simple->Nl[var];
 //     Simple->LowLev[var] = 0;
 //     printf("myv5dopen: var = %d, name = %s\n", var, Simple->VarName[var] );
      strncpy(  varname[var], Simple->VarName[var], 10 );
 //     Simple->VarName[var][9] = 0;
   }

   /* time and date for each timestep */
   for (time=0;time<*numtimes;time++) {
      timestamp[time] = Simple->TimeStamp[time];
      datestamp[time] = Simple->DateStamp[time];
   }

   *compressmode = Simple->CompressMode;

   /* Map projection and vertical coordinate system */
   *projection = Simple->Projection;
//   memcpy( Simple->ProjArgs, proj_args, MAXPROJARGS*sizeof(float) );

   *vertical = Simple->VerticalSystem;
 //     printf("v5dopen: NumVars = %d\n",  *numvars );
 //     printf("v5dopen: myumVar = %d\n",  mynumvar );
 //     *numvars = mynumvar;
 //     printf("v5dopen: Nr = %d, Nc = %d\n",  *nr, *nc );
